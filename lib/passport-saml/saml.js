...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  .then(certs => {
    ...
    if (encryptedAssertions.length == 1) {
      ...
      var xmlencOptions = {
        key: this.options.decryptionPvk,
        disallowDecryptionWithInsecureAlgorithm: true,
      };

      return Q.ninvoke(xmlenc, 'decrypt', encryptedAssertionXml, xmlencOptions)
      .then(decryptedXml => {
        // This `try` block is here as a deprecation notice to insecure algorithms.
        // It should be removed with the next semver major release
        try {
          let testDecryptedDoc = new xmldom.DOMParser().parseFromString(decryptedXml);
        } catch (e) {
          console.warn(e.message);
          xmlencOptions.disallowDecryptionWithInsecureAlgorithm = false;
        }
